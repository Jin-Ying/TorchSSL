...
class FlexMatch_2s:
    def __init__(self, net_builder, num_classes, ema_m, T, p_cutoff, lambda_u, \
        ...
        self.model_ta = net_builder(num_classes=num_classes)
        self.ema_model = None
    ...
    def set_optimizer(self, optimizer, optimizer_ta, scheduler=None, scheduler_ta=None):
        self.optimizer = optimizer
        self.optimizer_ta = optimizer_ta
        self.scheduler = scheduler
        self.scheduler_ta = scheduler_ta
    ...
    def train(self, args, logger=None):
        ...
        for (_, x_lb, y_lb), (x_ulb_idx, x_ulb_w, x_ulb_s) in zip(self.loader_dict['train_lb'],
            ...
            with amp_cm():
                logits_ta = self.model_ta(inputs)
                logits_ta_x_lb = logits_ta[:num_lb]
                logits_ta_x_ulb_w, logits_ta_x_ulb_s = logits_ta[num_lb:].chunk(2)
                ...
                unsup_loss, mask, select, pseudo_lb, p_model = consistency_loss(logits_ta_x_ulb_s,
                                                                                logits_ta_x_ulb_w,
                                                                                classwise_acc,
                                                                                p_target,
                                                                                p_model,
            ...
            if args.amp:
                ...
                if (args.clip > 0):
                    torch.nn.utils.clip_grad_norm_(self.model_ta.parameters(), args.clip)
                scaler.step(self.optimizer_ta)
                scaler.update()
            else:
                unsup_loss.backward()
                if (args.clip > 0):
                    torch.nn.utils.clip_grad_norm_(self.model_ta.parameters(), args.clip)
                self.optimizer_ta.step()
            
            self.scheduler_ta.step()
